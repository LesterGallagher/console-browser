...
key_value_node *create_key_value_node(json_type type, writing_mode mode, char *key, key_value_node *curr) {
    ...
    new_node->type = type;
    if (mode == WM_JSON_ARRAY) sprintf(key, "%lu", curr->val_len);
    new_node->key = malloc(strlen(key) + 1);
    strcpy(new_node->key, key);
    new_node->parent = curr;
    new_node->val_len = type == JSON_OBJECT || type == JSON_ARRAY ? 0 : 1;
    new_node->value = malloc(sizeof(key_value) * (type == JSON_OBJECT || type == JSON_ARRAY ? MAX_VALUES_SIZE : 1));
    return new_node;
}
    ...
    for (unsigned int i = 0; i < len; i++) {
        ...
        case BEFOREVALUEWRITE:
            ...
            if (s == '"') {
                ...
                key_value_node *str_node = create_key_value_node(JSON_STRING, mode, key, curr);
                ...
                key_value_node *num_node = create_key_value_node(JSON_NUMBER, mode, key, curr);
                ...
                key_value_node *bool_node = create_key_value_node(JSON_BOOLEAN, mode, key, curr);
                ...
                key_value_node *null_node = create_key_value_node(JSON_NULL, mode, key, curr);
                ...
                key_value_node *new_obj_node = create_key_value_node(JSON_OBJECT, mode, key, curr);
                if (curr != NULL) curr->value[curr->val_len++].node_val = new_obj_node;
                curr = new_obj_node;
                mode = WM_JSON_OBJECT;
                ws = WAITFORKEY;
            } else if (s == '[') {
                key_value_node *new_arr_node = create_key_value_node(JSON_ARRAY, mode, key, curr);
